...
/************************************************************/
namespace Exercise1.DataAccess.EFCore
{
    public static class EFExtensions
    {
        ...
        /// <returns></returns>
        public static DbCommand LoadStoredProc(this DbContext context, 
        ...
        public static DbCommand WithSqlParams(this DbCommand cmd, 
        ...
        public class SprocResults
        {
            ...
            public SprocResults(DbDataReader reader, DbContext context)
            {
                this._reader = reader;
            ...
            public IList<T> ReadToList<T>() where T : new()
            {
                ...
                return MapToList<T>(_reader, entityType);
            }
            ...
            private static IList<T> MapToList<T>(DbDataReader dr, IEntityType entityType) 
            where T : new()
            {
                var objList = new List<T>();
                var props = typeof(T).GetRuntimeProperties().ToList();

                var colNameMapping = entityType.GetProperties()
                    .ToDictionary(
                        k => k.GetColumnName(),
                        v => v.Name
                    );
                ...
                var colMapping = dr.GetColumnSchema()
                    .Where(x => props.Any(y => string.Equals(
                        y.Name, 
                        colNameMapping[x.ColumnName], 
                        StringComparison.CurrentCultureIgnoreCase)
                    ))
                    .ToDictionary(
                        k => colNameMapping[k.ColumnName].ToUpper(),
                        v => v
                    );
                ...
                while (dr.Read())
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        //     continue;
                        // var val = dr.GetValue(column.ColumnOrdinal.Value - 1);

                        if (column?.ColumnName == null)
                            continue;
                        var val = dr.GetValue(column.ColumnName);
        ...
        public static async Task ExecuteStoredProcAsync(this DbCommand command,
            ...
            bool manageConnection = true, 
            params Action<SprocResults>[] resultActions)
        {
        ...
        /// <param name="manageConnection"></param>
        /// <returns></returns>
        public static int ExecuteStoredNonQuery(this DbCommand command, 
            bool manageConnection = true)
        {
        ...
        /// <param name="manageConnection"></param>
        /// <returns></returns>
        public static async Task<int> ExecuteStoredNonQueryAsync(this DbCommand command, 
            CancellationToken ct = default,
            bool manageConnection = true)
        {
            var numberOfRecordsAffected = -1;
